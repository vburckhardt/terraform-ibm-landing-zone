...
The landing zone module can be utilized to create a fully customizable VPC environment within a single region. The five following patterns serve as starting templates, allowing for a quick start with Landing Zone. These patterns are stored in the [patterns](/patterns/) directory.
...
1. Enable virtual routing and forwarding (VRF) and service endpoints by creating a support case. Follow the instructions in enabling VRF and service endpoints at [this link](https://cloud.ibm.com/docs/account?topic=account-vrf-service-endpoint&interface=ui#vrf).
...
1.  Set up access groups [click here](https://cloud.ibm.com/docs/account?topic=account-account-getting-started#account-gs-accessgroups).
    User access to IBM Cloud resources is regulated through the use of access policies assigned to access groups. For IBM Cloud Financial Services validation, all IAM users must not be granted direct access to any IBM Cloud resources.
...
Through Secure Landing Zone, you can optionally provision the F5 BIG-IP, enabling the implementation of a client-to-site VPN or web application firewall (WAF). For further information, refer to [Provisioning a F5 BIG-IP host by using Secure Landing Zone](.docs/f5-big-ip/f5-big-ip.md).
...
The following IBM Cloud services can be configured from this module:
...
Cloud Object Storage components are defined in the [cos.tf](cos.tf) file.
