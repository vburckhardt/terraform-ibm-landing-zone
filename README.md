...
![latest release](https://img.shields.io/github/v/release/terraform-ibm-modules/terraform-ibm-landing-zone?logo=GitHub&sort=semver)
![Renovate enabled](https://img.shields.io/badge/renovate-enabled-brightgreen.svg)

The landing zone module can be utilized to create a fully customizable VPC environment within a single region. The five following patterns serve as starting templates, facilitating quick initiation with Landing Zone. These patterns are stored in the [patterns](/patterns/) directory.

Each of these patterns (excluding VSI QuickStart) generates the following infrastructure:
...
Complete the following steps before deploying the Secure Landing Zone module.
### Set up an IBM Cloud Account

1. Ensure you have an IBM Cloud Pay-As-You-Go or Subscription account:
...
1.  Set up access groups [click here](https://cloud.ibm.com/docs/account?topic=account-account-getting-started#account-gs-accessgroups).
    User access to IBM Cloud resources is regulated through the use of access policies assigned to access groups. For IBM Cloud Financial Services validation, all IAM users must not be directly assigned access to any IBM Cloud resources.
...
The second approach is to utilize the `override.json` file to create a fully customized environment based on the starting template. By default, each pattern's `override.json` contains the default environment configuration. You can use the `override.json` file in the respective pattern directory by setting the template input `override` variable to `true`. Each value in `override.json` corresponds directly to a variable value from this root module, which each pattern uses to create your environment.
...
Following every execution of `terraform apply`, a JSON-encoded definition is output. This definition of your environment is based on the defaults for the Landing Zone and any variables that are changed in the `override.json` file. You can then use the output in the `override.json` file.
...
Secure Landing Zone can set up a bastion host utilizing Teleport. For further details, see [Provisioning a bastion host using Teleport with Secure Landing Zone](.docs/bastion/bastion.md).
